...
L23~25:引数の数を決めて、引数の足りない場合をはじく。はじいた場合"error:too few arguments"を表示

L27~29:それぞれ必要な値を読み込む

L38:Num of dummy dataの回数分繰り返す
...
[comment #20200702]
- コマンドライン引数が足りなかったときに終了するようにしているのは良いですが, なぜ終了したかわかるように, 何らかのエラーメッセージを出した方が良いと思います. 
- l.35についてのソースコードの説明ですが, `5`回と書くより`Num of dummy data`の数とかにした方が良いです. コマンドライン引数の値を変えるとループ回数が変わるので. 

[comment #20200703]
- エラーメッセージは `error` だけでなく `too few arguments` のようにエラー内容がわかるようにした方がよいです. 
- エラーメッセージは追加されていますが, 異常終了部分が消えてしまっています. 
